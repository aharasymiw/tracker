...
async function installPasskeyHarness(page: import('@playwright/test').Page) {
  const fixtures = makeCredentialFixtures()

  await page.context().addInitScript(({ credentialId, prfOutput }) => {
...
async function launchPersistentApp(userDataDir: string) {
  const context = await chromium.launchPersistentContext(userDataDir, {
    headless: true,
  })
  const page = await context.newPage()
  await installPasskeyHarness(page)
  await page.goto(BASE_URL, { waitUntil: 'domcontentloaded' })
  void enableVirtualAuthenticator(page).catch(() => {})
  return { context, page }
}

test('passkey signup unlocks the vault and stay logged in survives reload and browser restart', async () => {
  test.setTimeout(120_000)

  const userDataDir = mkdtempSync(join(tmpdir(), 'trellis-passkey-e2e-'))
  let { context, page } = await launchPersistentApp(userDataDir)
  ...
  ;({ context, page } = await launchPersistentApp(userDataDir))
